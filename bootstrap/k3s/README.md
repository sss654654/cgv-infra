# bootstrap/k3s — 손으로 하는 구간 (k3s 클러스터 · 허브)
...
③ ★ SealedSecret 18종 봉인 → 커밋 → push        ← 사람만 할 수 있는 구간
      ...
      초기 10종(seal-secrets.sh) + 낱개 8종
        ...
        image-updater-registry   GitLab 레지스트리 태그 폴링 자격     위와 같은 dockerconfigjson
...
같이 놓여 있어서 이름만으로 어느 쪽인지 갈려야 한다.

`argocd-values.yaml` 은 이 폴더에서 유일하게 **살아 있는 Application 이 읽는 파일**이다.
ArgoCD 가 자기 자신을 이 값으로 관리한다(`argocd/applications/argocd.yaml`) — 이름을 바꾸면
그 Application 의 경로도 같은 커밋에서 고쳐야 한다.
